...
if __name__ == '__main__':
	...
	#################################################################################
	######## Prepare output directories, temporal smapling, reconstruct mesh  #######
	#################################################################################
	...
	with h5py.File(mesh_path, 'r') as hf:
		...
		#depending on pyvista and VTK version, the line below can sometimes run into errors.
		# I forget the workaround, but usually having the most recent pyvista version in conda environment will be fine 
	...
	#################################################################################
	############## Interactive Mesh clipping / ROI Specification ####################
	#################################################################################
	...
	for i,timestep_file in enumerate(timesteps_to_read):
		...
		if 'u' in quantities or 'q' in quantities:
			...
			#for the line below, not sure if this has to be done at each timestep, could be an optimization to define this once after bounding box specification
	...
	if 'path' in quantities:
		...
		#please read README diescription of `track_length`, which will make this sampling a little more clear
		#There's a difference between *photographing* the flow and the frames that actually get *written*
		#This is becasue of limitations of Horos, which can only take 100 timesteps, i.e., the flow "imaging" is constrained by `n_steps`
		...
		#have to read in an arbitrary sample mesh to get geo/bounds/ghosted lumen
		...
		for t,timestep in enumerate(particle_time_series_files[:-1]):
			...
			if t in photographed_frames and t in frames_to_write:
				...
				'''
				#uncomment this block (and move to appropriate place) if you want to instead write the volume to a .vti for rendering in paraview
